...
import cv2
from nets import MSSL_h36m as MSSL
...
tf.app.flags.DEFINE_integer('jointsdim', 3,
                            'input image width.')
tf.app.flags.DEFINE_integer('jointsnum', 22,
                            'joint number to predict.')
tf.app.flags.DEFINE_integer('totaljointsnum', 32,
...
tf.app.flags.DEFINE_integer('Tu_length', 1,
                            'The number of SE in BSME blocks.')

tf.app.flags.DEFINE_string('num_hidden', '64',
                           'number of units in a BSME.')
            ...
            for s in str1:
                ...
                for i in range(FLAGS.seq_length - FLAGS.input_length):
                    ...
                    for j in range(FLAGS.batch_size * FLAGS.n_gpu):
                        tem1=0
                        for k in range(test_ims1.shape[2]):
                            tem1 += np.sqrt(np.square(x[j,k] - gx[j,k]).sum())
                        mpjpe1[0,i] += tem1/test_ims1.shape[2]
